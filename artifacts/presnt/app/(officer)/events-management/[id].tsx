...
export default function OfficerEventDetailScreen() {
  ...
  // ── Geofence lifecycle ────────────────────────────────────────────────────
  // When the event is ongoing and has coordinates, register a geofence region
  // so members are auto-checked-in on arrival. Unregister when it ends.
  useEffect(() => {
    if (!event || !id) return;
    ...
    if (isOngoing) {
      // Register geofence region so the background task fires for members
