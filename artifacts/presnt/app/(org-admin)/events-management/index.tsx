/**
 * Org Admin — Events Management
 *
 * Lists all events across the entire organization (all chapters + org-wide events).
 * Events created here are org-wide (is_org_wide = true, org_id = parent org id).
 *
 * Chapter filter bar: "All" shows everything; selecting a chapter shows that
 * chapter's events + org-wide events. Org-wide events show a badge.
 *
...
function nextOccurrenceAfter(rruleStr: string, startIso: string, now: Date): Date | null {
  try {
    ...
    const until    = params['UNTIL'] ? new Date(params['UNTIL'].replace(/^(\d{4})(\d{2})(\d{2}).*/, '$1-$2-$3')) : null;
    ...
    if (freq === 'MONTHLY' && byDay.length === 1 && /^-?\d/.test(byDay[0])) {
      ...
      let yr = base.getFullYear(), mo = base.getMonth(), n = 0;
      while (true) {
        ...
        n++; mo += interval; yr += Math.floor(mo / 12); mo = mo % 12;
      }
    }

    let occ = new Date(base), n = 0;
    while (occ <= limit) {
      if (until && occ > until) return null;
      if (count !== null && n >= count) return null;
      let matches = true;
      ...
      if (freq === 'WEEKLY') {
        ...
        } else { occ = new Date(occ.getTime() + 7 * interval * 24 * 3600 * 1000); n++; }
...
function OrgWideBadge({ small = false }: { small?: boolean }) {
  return (
    <View style={{
...
function PickerBtn({ label, value, icon, onPress, color }: {
  label: string; value: string; icon: keyof typeof Ionicons.glyphMap; onPress: () => void; color: string;
}) {
  const { theme } = useThemeStore();
  const c = theme.colors;
  return (
...
function EventForm({
  ...
  useEffect(() => {
    if (visible) {
      ...
      setSavedLoc(null);
      onClearCodeError?.();
      if (!initial && !f.event_code) generateCode(f.dateObj);
    }
  }, [visible, initial]);
  ...
  const settingsPanel = (
    <View style={[ef.panel, { backgroundColor: c.surface, borderColor: c.border }]}>
      {/* Org-wide banner */}
      <View style={{ backgroundColor: '#3B82F610', borderBottomWidth: 1, borderBottomColor: '#3B82F630', padding: 14, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        <Ionicons name="globe-outline" size={16} color="#3B82F6" />
        <View style={{ flex: 1 }}>
          <Text size="sm" weight="semibold" color="#3B82F6">Organization Wide</Text>
          <Text size="xs" color={c.textMuted} style={{ marginTop: 1 }}>
            Visible to all members across every chapter
          </Text>
        </View>
      </View>
        ...
        {([
          ...
          <View key={key} style={ef.toggleRow}>
            ...
            <Switch value={!!form[key]} onValueChange={(v) => set(key)(v as any)}
              trackColor={{ false: c.border, true: '#3B82F6' }} thumbColor="#fff" />
          </View>
        ))}
      </View>
      ...
      <View style={[ef.panelSection, { borderTopWidth: 1, borderTopColor: c.border, paddingTop: 14 }]}>
        ...
        <Text size="xs" color={c.textSubtle} style={{ marginBottom: 5 }}>Opens before start (minutes)</Text>
        <View style={[ef.checkinRow, { borderColor: c.border, backgroundColor: c.surfaceAlt }]}>
          ...
          <TextInput style={{ flex: 1, fontSize: 14, color: c.text }} value={form.checkin_open_minutes}
            onChangeText={set('checkin_open_minutes')} keyboardType="number-pad" placeholder="e.g. 15"
            placeholderTextColor={c.textSubtle} />
          <Text size="xs" color={c.textSubtle}>min</Text>
        </View>
        <Text size="xs" color={c.textSubtle} style={{ marginTop: 10, marginBottom: 5 }}>Grace period after start/end (minutes)</Text>
        <View style={[ef.checkinRow, { borderColor: c.border, backgroundColor: c.surfaceAlt }]}>
          <Ionicons name="hourglass-outline" size={15} color={c.textSubtle} />
          <TextInput style={{ flex: 1, fontSize: 14, color: c.text }} value={form.checkin_grace_minutes}
            onChangeText={set('checkin_grace_minutes')} keyboardType="number-pad" placeholder="e.g. 15"
            placeholderTextColor={c.textSubtle} />
          <Text size="xs" color={c.textSubtle}>min</Text>
        </View>
      </View>
      ...
      <View style={[ef.panelSection, { marginTop: 0, borderTopWidth: 1, borderTopColor: c.border, paddingTop: 14 }]}>
        <Text size="xs" weight="medium" color={c.textMuted}
          style={{ textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Points</Text>
    ...
    return (
      <View style={[ef.card, { backgroundColor: c.surface, borderColor: c.border }]}>
        ...
        <TextInput style={[inputStyle, { marginBottom: 12, paddingVertical: 14, fontSize: 16 }]} value={form.title}
          onChangeText={set('title')} placeholder="e.g. Spring Formal 2026"
          placeholderTextColor={c.textSubtle} />
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
          ...
          <Switch value={form.is_public} onValueChange={(v) => { set('is_public')(v); onClearCodeError?.(); }}
            trackColor={{ false: c.border, true: '#3B82F6' }} thumbColor="#fff" />
        </View>
          ...
          <View style={{ flex: 1, zIndex: 100 }}>
            ...
            {showCatMenu && (
              <View style={[ef.dropdown, { backgroundColor: c.surface, borderColor: c.border }]}>
                <Pressable onPress={() => { set('category_id')(null); setShowCatMenu(false); }}
                  style={[ef.dropdownItem, { borderBottomColor: c.border }]}>
                  <Text size="sm" color={c.textMuted}>None</Text>
                </Pressable>
                {categories.map(cat => (
                  <Pressable key={cat.id} onPress={() => { set('category_id')(cat.id); setShowCatMenu(false); }}
                    style={[ef.dropdownItem, { borderBottomColor: c.border }]}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                      <View style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: cat.color }} />
                      <Text size="sm" color={form.category_id === cat.id ? '#3B82F6' : c.text}
                        weight={form.category_id === cat.id ? 'medium' : 'regular'}>{cat.name}</Text>
                    </View>
                  </Pressable>
                ))}
              </View>
        ...
        <View style={{ position: 'relative', zIndex: (showDate || showRange) ? 500 : 1 }}>
          {form.isMultiDay ? (
            <PickerBtn label="Date range" value={formatDateRange(form.dateRange.start, form.dateRange.end)}
              icon="calendar-outline" color="#3B82F6" onPress={() => setShowRange(true)} />
          ) : (
            <>
              <PickerBtn label="Date" value={formatDateDisplay(form.dateObj)}
                icon="calendar-outline" color="#3B82F6" onPress={() => setShowDate(!showDate)} />
              <DatePickerModal visible={showDate} value={form.dateObj}
                onConfirm={(d) => {
                  const newDate = new Date(d);
                  const newStart = new Date(newDate); newStart.setHours(form.startTimeObj.getHours(), form.startTimeObj.getMinutes(), 0, 0);
                  const newEnd = new Date(newDate); newEnd.setHours(form.endTimeObj.getHours(), form.endTimeObj.getMinutes(), 0, 0);
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginTop: 10 }}>
          <View style={{ flex: 1 }}>
            ...
            <PickerBtn label="Start" value={formatTimeDisplay(form.startTimeObj)}
              icon="time-outline" color="#3B82F6" onPress={() => setShowStartTime(true)} />
          </View>
          <View style={{ flex: 1 }}>
        ...
        <View style={{ marginTop: 10 }}>
          ...
          <PickerBtn label="Recurrence" value={rruleSummary}
            ...
            color={form.recurrence.freq === 'none' ? c.textSubtle : '#3B82F6'}
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 14 }}>
          {(['in-person', 'remote'] as const).map((mode) => {
            ...
            return (
              <Pressable key={mode} onPress={() => set('location_type')(mode)}
                style={[ef.modeChip, {
        ...
        {form.location_type === 'remote' ? (
          <>
            ...
            <View style={[ef.urlRow, { backgroundColor: c.surfaceAlt, borderColor: form.meeting_url ? '#3B82F6' : c.border }]}>
              <Ionicons name="link-outline" size={16} color={form.meeting_url ? '#3B82F6' : c.textSubtle} />
              <TextInput style={{ flex: 1, fontSize: 14, color: c.text }} value={form.meeting_url}
                onChangeText={set('meeting_url')} placeholder="https://zoom.us/j/…"
                placeholderTextColor={c.textSubtle} autoCapitalize="none" autoCorrect={false} keyboardType="url" />
              {!!form.meeting_url && (
          ...
          <>
            <Pressable onPress={() => setShowLocPicker(true)}
              ...
              <Text size="xs" weight="medium" color="#3B82F6">
        ...
        {isEdit && isWide && (
          <Pressable onPress={handleDelete}
            ...
            <Text size="sm" weight="medium" color="#EF4444">
              {(initial as any)?.recurrence_rule ? 'Delete event…' : 'Delete event'}
            </Text>
          </Pressable>
        )}
      </View>
  ...
  return (
    <>
        ...
        <View style={{ flex: 1, backgroundColor: c.background }}>
          <View style={[ef.header, { backgroundColor: c.surface, borderBottomColor: c.border, paddingTop: insets.top + 14 }]}>
            ...
            <View style={{ flex: 1, alignItems: 'center' }}>
              <Text size="md" weight="bold">{isEdit ? 'Edit event' : 'New org-wide event'}</Text>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 2 }}>
                <Ionicons name="globe-outline" size={10} color="#3B82F6" />
                <Text size="xs" color="#3B82F6">Organization Wide</Text>
              </View>
            </View>
            <Pressable onPress={() => onSave(form)} disabled={saving || !form.title.trim()}
              style={[ef.postBtn, { backgroundColor: '#3B82F6', opacity: saving ? 0.6 : 1 }]}>
              {saving
      ...
      <Modal visible={confirmDelete !== null} transparent animationType="fade" onRequestClose={() => setConfirmDelete(null)}>
        <Pressable style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 24 }}
          ...
          <Pressable onPress={(e) => e.stopPropagation()}>
            <View style={{ backgroundColor: c.surface, borderRadius: 16, padding: 24, width: 320, gap: 12 }}>
              ...
              <Text size="sm" color={c.textMuted} style={{ lineHeight: 20 }}>
                {confirmDelete === 'recurring' ? 'Delete this occurrence or the entire series?' : 'This cannot be undone.'}
              </Text>
              <View style={{ gap: 8, marginTop: 4 }}>
                {confirmDelete === 'recurring' ? (
                  <>
                    <Pressable onPress={() => { setConfirmDelete(null); onDelete(initial!, 'this'); }}
                    ...
                    <Pressable onPress={() => { setConfirmDelete(null); onDelete(initial!, 'series'); }}
      ...
      <LocationPickerModal visible={showLocPicker} orgId={orgId} selectedId={form.location_id}
        onSelect={(loc) => {
          ...
          setForm(f => ({ ...f, location_id: loc?.id ?? null, location_text: loc?.name ?? f.location_text,
            location_lat: loc?.latitude ?? f.location_lat, location_lng: loc?.longitude ?? f.location_lng }));
      ...
      <MapPickerModal visible={showMapPicker}
        ...
        onConfirm={({ lat, lng, address }) => {
          setForm(f => ({ ...f, location_lat: lat || null, location_lng: lng || null, location_text: address || f.location_text }));
...
const ef = StyleSheet.create({
  ...
  deleteBtn:   { borderWidth: 1, borderRadius: 12, paddingVertical: 14, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 6 },
    ...
    >
      ...
      <View style={{ flex: 2, gap: 2 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          ...
          {(event as any).recurrence_rule && (
            <View style={[tr.recurBadge, { backgroundColor: '#3B82F618', borderColor: '#3B82F650' }]}>
              <Ionicons name="repeat" size={9} color="#3B82F6" />
              <Text size="xs" color="#3B82F6">Recurring</Text>
            </View>
          )}
        </View>
      </View>
      <View style={{ flex: 1 }}>
        {(event as any).meeting_url ? (
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
            <Ionicons name="videocam-outline" size={13} color="#3B82F6" />
            <Text size="sm" color="#3B82F6" numberOfLines={1}>Remote</Text>
          </View>
        ) : (
        ...
        <View style={{ flex: 1, gap: 3 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
            ...
            {(event as any).meeting_url ? (
              ...
              <Text size="xs" color="#3B82F6">Remote</Text></>
            ) : event.location ? (
              <><Text size="xs" color={c.textSubtle}>·</Text>
...
export default function OrgAdminEventsScreen() {
  const { theme }                    = useThemeStore();
  const insets                       = useSafeAreaInsets();
  ...
  const load = useCallback(async () => {
    ...
    // Load chapters under this parent org
    ...
    // Load org-wide events (org_id = parentOrgId) + chapter events
    // org-wide events have is_org_wide = true and org_id = parentOrgId
  ...
  const filtered = events.filter(e => {
    ...
    if (chapterFilter !== null) {
      const isOrgWide  = !!(e as any).is_org_wide;
      const belongsTo  = e.org_id === chapterFilter;
      if (!isOrgWide && !belongsTo) return false;
    }
    // Status tab filter
  ...
  async function handleSave(form: EventFormState) {
    ...
    try {
      ...
      if (editingEvent?.id) {
        ...
        // New org-wide event: org_id = parentOrgId, is_org_wide = true
  ...
  async function handleDelete(event: Event, mode: 'this' | 'series') {
    ...
    try {
      if (mode === 'series') {
        ...
        await supabase.from('events').update({ is_deleted: true }).eq('id', masterId);
        await supabase.from('events').update({ is_deleted: true }).eq('parent_event_id', masterId);
      } else {
        await supabase.from('events').update({ is_deleted: true }).eq('id', event.id);
      }
      await load();
      setEditing(false);
    } catch (err: any) {
    ...
    <View style={{ flex: 1, backgroundColor: c.background }}>
      ...
      <View style={[sc.header, { paddingTop: isWide ? 20 : insets.top + 12, backgroundColor: c.background, borderBottomColor: c.border }]}>
        ...
        <View style={sc.headerActions}>
          <Pressable onPress={() => setEditing(null)} style={[sc.newBtn, { backgroundColor: '#3B82F6' }]}>
      ...
      {isWide && (
        <View style={[sc.tableHeader, { backgroundColor: c.background, borderBottomColor: c.border }]}>
          {[['EVENT', 2], ['LOCATION', 1], ['STATUS', 0]].map(([label, flex]) => (
        ...
        {filtered.length === 0 ? (
          <View style={sc.empty}>
            ...
            <Pressable onPress={() => setEditing(null)} style={{ marginTop: 12 }}>
              <Text size="sm" color="#3B82F6" weight="medium">+ Create org-wide event</Text>
            </Pressable>
          </View>
        ) : isWide ? (
          filtered.map(e => (
