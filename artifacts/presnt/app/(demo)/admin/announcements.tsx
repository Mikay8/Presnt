/**
 * Demo Admin — Announcements (read-only)
 *
 * Shows the announcement history list.
 * The compose card and "Send to Chapter" button are visible but
 * disabled (opacity 0.4, no onPress, TextInputs not editable).
 */
...
export default function DemoAdminAnnouncementsScreen() {
  ...
  function renderItem(item: Announcement, isLast: boolean) {
    const date = new Date(item.created_at);
    const label = date.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
    const time  = date.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' });
    return (
      <View key={item.id} style={[styles.histRow, { borderBottomColor: c.border }, isLast && { borderBottomWidth: 0 }]}>
      ...
      >
        ...
        <Card style={{ paddingVertical: 0 }}>
          {loadingHist ? (
            ...
            history.map((item, i) => renderItem(item, i === history.length - 1))
          )}
        </Card>
      </ScrollView>
    </View>
