/**
 * Demo Admin — Status (read-only)
 *
 * Full compliance status view (attendance + dues tabs).
 * All Supabase calls are .select() only.
 * The "+ Add requirements" button is removed — replaced with a
 * non-interactive disabled view.
 * Navigation links (e.g. router.push to requirements) are removed.
 */
...
export default function DemoAdminStatusScreen() {
  const { theme }    = useThemeStore();
  const c            = theme.colors;
