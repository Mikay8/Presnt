/**
 * Demo Admin — Dashboard (read-only copy of (admin)/dashboard.tsx)
 * No write operations. Quick-action buttons navigate within the demo portal.
 */
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.background }}>
      <View style={[
        ...
        {!isWide && (
          <Pressable
            onPress={() => router.push('/(demo)/admin/profile' as any)}
      ...
      >
        ...
        {/* Quick actions — navigate within demo portal */}
        <View style={styles.quickRow}>
          {[
            { icon: 'people-outline',        label: 'Members',     route: '/(demo)/admin/members'       },
            { icon: 'cash-outline',          label: 'Dues',        route: '/(demo)/admin/dues'          },
            { icon: 'alert-circle-outline',  label: 'Status',      route: '/(demo)/admin/status'        },
            { icon: 'megaphone-outline',     label: 'Announce',    route: '/(demo)/admin/announcements' },
          ].map((item) => (
            <Pressable
              key={item.label}
