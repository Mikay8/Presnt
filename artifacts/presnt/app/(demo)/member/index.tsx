/**
 * Demo Member — Home (read-only)
 *
 * Shows announcements, upcoming events, and attendance donut.
 * Notifications are read-only (no mark-read writes).
 * The "Submit excuse" button is completely removed.
 * "View calendar" routes to /(demo)/member/calendar.
 */
...
function NotificationRow({ item }: { item: AppNotification }) {
  ...
  return (
    <View style={[styles.notifRow, { borderBottomColor: c.border }, !item.is_read && { backgroundColor: c.primary + '08' }]}>
...
export default function DemoMemberHomeScreen() {
  const { theme }        = useThemeStore();
      ...
      <View style={[styles.wideRoot, { backgroundColor: theme.colors.background }]}>
        ...
        >
          <View style={styles.wideTitleRow}>
            ...
            {/* "Submit excuse" removed — only "View calendar" remains */}
            <View style={styles.wideTitleActions}>
              <Button label="View calendar" variant="outline" size="sm" onPress={() => {}} />
              <Pressable
