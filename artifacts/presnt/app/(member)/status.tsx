...
function HistoryRow({ item, isLast }: { item: AttendanceRecord; isLast: boolean }) {
  ...
  return (
    <View style={[styles.historyRow, !isLast && { borderBottomWidth: 1, borderBottomColor: theme.colors.border }]}>
      ...
      <View style={{ alignItems: 'flex-end', gap: 4 }}>
        ...
        {excused && <Text size="xs" color={theme.colors.textMuted}>Excused</Text>}
      </View>
    </View>
  );
}
...
export default function StatusScreen() {
  ...
  const load = useCallback(async () => {
    ...
    // 4. Attendance records (recent 50)
    ...
    // 5. Total mandatory events in term (for headline donut)
    if (termData) {
  ...
  // Overall status: at risk if any snapshot is at risk
